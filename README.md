...
This fork adds **IsoExec**: the Megatron trainer and the vLLM rollout engine compute
**bitwise-identical** token logprobs, so the PPO/GRPO importance ratio `r_t = π_train / π_rollout`
is exactly 1 and no KL correction term is needed.

Trainer and engine normally run different implementations of the same mathematics — a chunked scan
versus a recurrent one, a fused epilogue versus an unfused one. These are algebraically equal and
numerically different, and that difference surfaces as a spurious importance ratio. IsoExec removes
the difference rather than correcting for it, and makes the claim checkable: every op declares which
implementation it installs at each site, those declarations are frozen into a content-hashed
execution contract, and the two processes exchange hashes before weight sync — a composition that
differs across runtimes refuses to start instead of silently producing a wrong ratio.

The stack is inert unless `SKYRL_ISOEXEC=1`, so everything below behaves exactly as upstream SkyRL
when it is off.
