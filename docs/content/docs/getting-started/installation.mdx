...
If you're running on an existing Ray cluster (see [Running on an existing Ray cluster](#running-on-an-existing-ray-cluster) section), we suggest using Ray 2.51.1 and Python 3.12. However, we support Ray versions >= 2.44.0.
...
```bash
docker run -it  --runtime=nvidia --gpus all --shm-size=8g --name skyrl novaskyai/skyrl-train-ray-2.51.1-py3.12-cu12.8 /bin/bash
```
...
```bash
docker run -it  --runtime=nvidia --gpus all --shm-size=8g --name skyrl novaskyai/skyrl-train-ray-2.51.1-py3.12-cu12.8-megatron /bin/bash
```
...
If you wish to use SkyRL with ray != 2.51.1, see the guide for [Running on an existing Ray cluster](#running-on-an-existing-ray-cluster).
</Callout>
...
- CUDA 12.8
- [uv](https://docs.astral.sh/uv/)
- [ray](https://docs.ray.io/en/latest/) 2.51.1
