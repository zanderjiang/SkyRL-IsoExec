...
│   ├── config/         # dataclass + OmegaConf config (config.py, sft_config.py) -- no YAML
...
- **Backend selection**: `trainer.strategy` chooses backend — `fsdp` (default) or `megatron`. (There is no `jax` strategy: `main_base.py` and `skyrl_train_backend.py` accept only those two and raise otherwise. The JAX/TPU work lives in `skyrl/tx`, reached a different way.)
