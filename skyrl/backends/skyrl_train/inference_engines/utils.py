...
def build_engine_runtime_env(
    ...
    # Crash forensics (glm_sleep_fix 2026-08-09): a fatal signal in an engine actor or its vLLM
    # worker children otherwise dies SILENTLY (vLLM installs no faulthandler in mp workers, and
    # probe D's one DAPO wake crash left no exit reason anywhere on a node without readable
    # dmesg/journal/coredump). Forward the switch from the launch shell; it costs nothing until
    # a process crashes, at which point it prints the fatal-signal stack instead of nothing.
    # Not a isoexec flag (crash forensics is stack-agnostic), so a direct forward like
    # PYTORCH_CUDA_ALLOC_CONF above rather than a flags.py allowlist entry.
    if os.environ.get("PYTHONFAULTHANDLER"):
        env_vars["PYTHONFAULTHANDLER"] = os.environ["PYTHONFAULTHANDLER"]
    # SkyRL-IsoExec: the engine actors set their own runtime_env, so forward the IsoExec switch
    # here explicitly (don't rely on job-level env_vars merging) -- the GPTModel registration +
    # native weight sync only run when the engine actor sees SKYRL_ISOEXEC=1.
    if os.environ.get("SKYRL_ISOEXEC"):
        ...
        for _zk in _ix_fwd("engine"):
            if os.environ.get(_zk):
                env_vars[_zk] = os.environ[_zk]
    if extra_env_vars:
        env_vars.update(extra_env_vars)
    ray_python = os.environ.get("SKYRL_RAY_PY_EXECUTABLE")
    if ray_python:
        # Preserve the venv entrypoint path. Resolving its symlink selects the base interpreter and
        # loses the environment whose packages and loader paths this runtime_env is meant to pin.
        ...
        # Actor-specific runtime_env values replace rather than reliably merge the job-level
        # environment on every Ray release. Preserve the exact interpreter's loader and source
        # roots explicitly for engine actors and their nested vLLM workers.
        for runtime_key in (
            "CUDA_HOME",
            "CUDA_LIB_PATH",
            "CUDNN_PATH",
    ...
    if not env_vars and not ray_python:
        return None
