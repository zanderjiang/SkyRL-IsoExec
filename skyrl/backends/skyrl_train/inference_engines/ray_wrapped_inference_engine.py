...
class RayWrappedInferenceEngine(InferenceEngineInterface):
    ...
    async def isoexec_reapply_cached_weights(self):
        return await self.inference_engine_actor.isoexec_reapply_cached_weights.remote()
...
def create_ray_wrapped_inference_engines(
    ...
    if inference_engine_enable_sleep:
        ...
        # SkyRL-IsoExec: force level 1 (offload weights to CPU + restore on wake) so the
        ...
        # (engine weights -> norm 0.0 -> uniform/gibberish generation).
        import os as _os

        sleep_level = 1 if (enable_lora or _os.environ.get("SKYRL_ISOEXEC") == "1") else sleep_level
