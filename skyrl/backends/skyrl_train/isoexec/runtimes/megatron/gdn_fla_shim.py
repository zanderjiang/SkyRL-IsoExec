...
Registers a facade ``fla`` in ``sys.modules`` forwarding ``chunk_gated_delta_rule`` / ``l2norm`` /
``causal_conv1d`` to ``isoexec.ops.gdn``, plus eager rebinds of the ``jit_fuser`` helpers. Megatron
binds these at import time, so :func:`install_fla_shim` MUST run before megatron.core is imported.
"""
...
def _scoring_gate_view(gate):
    ...
    Uses ``view`` rather than ``reshape`` so an unsupported layout returns ``None`` instead of
    silently allocating the copy this path exists to remove.
    """
    if gate.ndim < 3:
        return None
...
def _l2norm_subsumed_by_core() -> bool:
    """True iff the selected GDN core does l2norm in-kernel.

    Every standalone l2norm site must then be an identity passthrough, or the forward
    double-normalizes. Single source for that decision.
    """
...
def _shim_chunk_gated_delta_rule(
    ...
    **_ignored,
    ...
    A custom ``scale`` is rejected rather than ignored: it would make trainer and engine differ.
    ``gdn_core`` dispatches on ``SKYRL_ISOEXEC_GDN_KERNEL``, so under ``recurrent`` this entry point
    runs the recurrent scan despite its name.
    """
    ...
    if _l2norm_subsumed_by_core():
        # Native composition: l2norm and gating happen in-kernel, so q/k arrive raw and g/beta are
        # the raw (alpha, b) with (A_log, dt_bias) stashed by _compute_g_and_beta.
...
def _no_recurrent(*_a, **_k):
    ...
    The recurrent kernel must be reached through ``gdn_ops``, which supplies the state pool and slot
    index map the bare FLA entry point does not.
    """
...
# Gating is computed immediately before the core call, so the stash holds at most one entry.
_GATING_STASH: list = []
...
def _patch_megatron_checkpoint_fp8_default() -> bool:
    ...
    Upstream's ``self.fp8 = fp8 is not None`` is truthy for the ``fp8=False`` default, so the forward
    takes the TE-only branch and NameErrors without TE. Normalise ``False`` to mean "no fp8".
    """
    try:
...
def _patch_megatron_gdn_native_gating() -> bool:
    ...
    Passes (alpha, b) through and stashes (A_log, dt_bias) so the fused kernel computes the gating
    itself, as the engine does; Megatron's own bf16 ``exp(A_log)`` differs in the last ulp.
    """
    try:
...
def _shim_causal_conv1d(
    ...
    **_ignored,
    ...
    ``x`` is ``[B, T, D]``, ``weight`` ``[D, W]``; returns ``(y, final_state)``. With ``cu_seqlens``
    each sequence is convolved on its own so the conv never leaks across a packed boundary.
    """
    import torch
    ...
    if (gdn_native_kernels_enabled() and (recurrent_mode() or cpr_mode())) or (
        ...
        # Its fp32 bias rounding differs from the eager conv below, so trainer and engine switch
        # together. B>1 flattens to varlen with uniform lengths.
        if initial_state is not None or output_final_state:
...
def _eager_compute_g_and_beta(self, A_log_local_cp, dt_bias_local_cp, alpha, beta):
    ...
    Kept character-for-character (``exp`` in the parameter dtype, softplus in fp32) so it rounds the
    same way as ``gdn_ops.gdn_gate_and_beta``.
    """
...
def _eager_prepare_qkv(self, qkv, gate, beta, alpha, batch, seq_len):
    ...
    if self.use_qk_l2norm:
        # The standalone normalisation site: runs only when the core does not subsume it.
    ...
    # The scoring fused-outnorm accepts the token-strided gate view; every other path keeps the
    # contiguous copy.
    gate_out = (
        gate
...
def _eager_apply_gated_norm(self, x, gate):
    ...
    Inductor shape-specializes the fused norm+silu+mul kernel, so decode and prefill would round the
    same row differently. Eager aten is shape-invariant.
    """
    # Scoring-only fast path: the Triton kernel is byte-exact but has no backward, so it needs both
    # eval mode and disabled autograd.
...
def _validate_once_enabled() -> bool:
    ...
    Off by default: collapsing the check deletes per-layer kernel launches, changing the launch
    sequence even though only discarded temporaries are involved.
    """
...
def _resolve_cu_seqlens_memo(self, cu_seqlens_padded, cu_seqlens_actual, total_seq_len, name, cp_size: int = 1):
    ...
    The function returns one of its inputs unchanged (everything else is validation), so the memo
    cannot change what the model sees. At CP=1 the divisibility check is a tautology and is skipped.
    """
    ...
    # Record the resolved (cu, cp_size) so `_unpack_sequence_memo` can recognise the
    # `cu_seqlens_q // self.cp_size` its call site builds. The monotone generation is the ledger key
    # because `id(cu)` aliases: CPython recycles tensor addresses once the old cu is dropped.
    if (
...
# One slot: every GDN layer of a forward resolves the same object. ``cu`` is held strongly.
...
# so an entry cannot be served to a different microbatch's cu_seqlens.
_DIV_PROOF: dict = {}
_DIV_PROOF_MAX = 16
...
def divided_cu_census() -> dict:
    """``{proved, served, reverified, disproved, unmatched, stale}``; ``stale`` must stay 0."""
    return dict(_DIV_STATS)


def _div_hoist_enabled() -> bool:
    """``SKYRL_ISOEXEC_PACKED_META_DIV_HOIST`` (default ON): kill switch for :func:`_divided_cu_list`."""
...
def _divided_cu_list(d, dim: int):
    ...
    The call site allocates a fresh tensor per GDN layer, so no identity memo can hit it; the first
    call each forward pays the real ``.tolist()`` to buy the proof, re-proven every
    ``_DIV_REVERIFY`` serves. Any unexpected condition falls through to the plain per-call read.
    """
    ...
    if not packed_meta_cache_enabled() or not _div_hoist_enabled() or cu is None or cp < 1:
        return d.tolist()
    # Structural precondition: `cu // cp` is a fresh tensor with cu's shape, dtype and device;
    # anything else is a caller this ledger has no claim on.
...
def _unpack_sequence_memo(x, cu_seqlens, dim=1):
    ...
    Otherwise the upstream body verbatim; this removes a pageable D2H and nothing else. The
    ``is``-check comes first because the direct call site can run after the LRU evicted this ``cu``.
    """
...
def _apply_rotary_pos_emb_thd_memo(
    ...
    ``seqlens`` is derived on the host from one memoized read and is elementwise equal to the device
    expression (exact integer arithmetic), so every downstream kernel sees identical arguments.
    """
    import torch
...
def _patch_megatron_packed_meta() -> bool:
    ...
    Call-time gated on ``SKYRL_ISOEXEC_PACKED_META_CACHE``, so installing unconditionally is free.
    """
    try:
        from megatron.core.ssm import gated_delta_net as mg
    except Exception as e:  # pragma: no cover - megatron absent (engine-only process)
...
def _module(name: str, **attrs) -> types.ModuleType:
    ...
    # `__spec__ = None` would make `importlib.util.find_spec(name)` raise ValueError, which
    # transformers calls on `fla`. Give the facade a real (loader-less) spec.
...
def install_fla_shim(*, force: bool = False) -> bool:
    ...
    Idempotent; a no-op unless ``SKYRL_ISOEXEC_GDN=1`` or ``force``. Refuses to shadow a real
    ``flash-linear-attention`` install, which would silently unshare the kernel.
    """
    global _installed
    ...
    # __version__ "0.0.0" makes `transformers.is_flash_linear_attention_available()` (which requires
    # >= 0.2.2) answer no, so HF uses its own torch reference instead of this facade. Megatron
    # imports our symbols by name and never consults that check.
    ...
    # Upstream megatron bug that only fires without TransformerEngine.
    ...
    # would abort while building the first layer.
    ...
    # so the boolean it returns cannot move. Default off, fail-soft.
    try:
    ...
    # Pin the Triton autotune configs if vLLM is importable here; `gdn_chunk` pins again before its
    # first launch, so a deferred pin is safe.
    try:
