"""Pytest bootstrap that keeps the ops/gdn CPU tests runnable without megatron.
...
lets ``...isoexec.ops.gdn.<module>`` resolve to the real files while the heavy ``__init__`` never
