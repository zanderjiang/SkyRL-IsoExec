    ...
    async def _send_chunks_legacy(self, chunks: Iterable[WeightChunk]) -> None:
        ...
        for chunk in chunks:
            ...
            if rank == 0:
                ...
                # Pack at the CHUNK's dtype, not unconditionally the model dtype -- same reasoning
                # as CudaIpcTransferStrategy: a dtype-homogeneous chunk may carry allowlisted
                # buffers in native precision (fp32 `mlp.router.expert_bias` steers MoE top-k, so
                # rounding it here changes which experts run). Mixed chunks keep the old behaviour.
                ...
                packed_tensor = torch.empty(total_numel, device=device, dtype=pack_dtype, requires_grad=False)
                offset = 0
                ...
                request = BroadcastWeightUpdateRequest(
                    names=chunk.names,
                    dtypes=[pack_dtype_str] * len(chunk),
                    shapes=chunk.shapes,
                    sizes=sizes,
                )
...
class BroadcastWeightTransferReceiver(WeightTransferReceiver):
    ...
    def receive_weights(self, request: BroadcastWeightUpdateRequest) -> Iterator[Tuple[str, torch.Tensor]]:
        ...
        if request.sizes is not None:
            ...
            # Allocate at the REQUEST's dtype. It is normally the model dtype, but a chunk carrying
            # allowlisted native-precision buffers (fp32 `mlp.router.expert_bias`) is sent in that
            # precision on purpose; asserting model dtype here would reject it, and allocating the
            # receive buffer as bf16 would round it. Consumers copy with an explicit `.to(dest.dtype)`.
            ...
            for name, dtype_str, shape in zip(request.names, request.dtypes, request.shapes):
                # Per-tensor dtype, for the same reason as the packed path above: an allowlisted
                # native-precision buffer is legitimately not the model dtype.
