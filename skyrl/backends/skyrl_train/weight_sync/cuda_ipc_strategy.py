...
class CudaIpcWeightTransferSender(WeightTransferSender):
    ...
    async def _send_chunks_legacy(self, chunks: Iterable[WeightChunk]) -> None:
        ...
        # TRAINER WAVE-3 / TARGET-2: overlap the engine-side apply of chunk k with the trainer-side
        # extraction+pack of chunk k+1. Measured baseline (35B-A3B, TP2/EP8/ETP1 -> engine TP8):
        # the engine's summed receiver.apply/install is ~9.2 s inside a ~24.5 s send loop that is
        # otherwise pure trainer work, and the two touch DISJOINT param sets -- chunk k's bytes are
        # already packed, IPC-exported and stream-synchronized before the apply is issued, and
        # chunk k+1's extraction reads Megatron params the engine never writes. So the serialization
        # is an artifact of awaiting the RPC inline, not a data dependency.
        #
        # BITWISE: nothing about the bytes changes. Same chunks, same packing, same per-tensor
        # copies, same order (exactly one apply is ever in flight). Only the ISSUE POINT of the
        # RPC moves. The two invariants the old loop enforced by construction are kept explicitly:
        #   (1) the packed buffer of chunk k stays alive (``prev_packed``) until chunk k's apply has
        #       RETURNED -- the receiver copies out of the sender's IPC mapping, so freeing early
        #       would hand it a dangling mapping;
        #   (2) ``ipc_collect`` for chunk k runs only after that same await, so the sender never
        #       reclaims a mapping the receiver still holds (the +8.7 GiB/3000-param leak).
        # A barrier follows the await, so every rank retires chunk k together exactly as before.
        _pipeline = _os.environ.get("SKYRL_ISOEXEC_SYNC_PIPELINE", "1") == "1"
        _pending = None  # rank 0 only: in-flight apply task for the PREVIOUS chunk
        _prev_packed = None  # every rank: previous chunk's packed buffer, kept mapped until retired
        _it = iter(chunks)
        while True:
            chunk = next(_it, None)
            if chunk is None:
            ...
            total_numel = sum(t.numel() for t in chunk.tensors)
            # PACK AT THE CHUNK'S OWN DTYPE, not unconditionally at the model dtype. A chunk may
            # legitimately carry tensors in a NON-model dtype: the IsoExec native sync ships
            # allowlisted BUFFERS in their native precision (`mlp.router.expert_bias` is fp32 and is
            # added to the MoE routing scores, so bf16-rounding it flips top-k membership -- a
            # different model, not a rounding difference). Packing into a bf16 staging buffer
            # rounded those values HERE, one line before the IPC export, silently undoing the
            # guarantee the extractor makes; the receiver then merely upcast the damage back to
            # fp32. Senders keep buckets dtype-homogeneous, so this is well defined; a mixed chunk
            # falls back to the model dtype exactly as before.
            _chunk_dtypes = {t.dtype for t in chunk.tensors}
            pack_dtype = _chunk_dtypes.pop() if len(_chunk_dtypes) == 1 else dtype
            packed_tensor = torch.empty(
                total_numel,
                device=device,
                dtype=pack_dtype,
            ...
            pack_dtype_str = str(pack_dtype).removeprefix("torch.")
            ...
            # ---- pipelined: retire chunk k-1 (its apply ran while we extracted+packed chunk k)
            if _pending is not None:
                await _pending
                _pending = None
            torch.distributed.barrier()
            _prev_packed = None  # release the retired chunk's IPC-exported buffer
            torch.cuda.ipc_collect()
            torch.cuda.synchronize()
            # ---- issue chunk k's apply WITHOUT awaiting it, then fall through to extract k+1
            if rank == 0:
                request = CudaIpcWeightUpdateRequest(
                    names=names,
                    dtypes=dtypes,
                ...
                _pending = asyncio.ensure_future(self._inference_client.update_named_weights(request))
                # Hand control to the loop just long enough for the task to reach its first real
                # suspension point -- i.e. for the Ray `.remote()` calls to be DISPATCHED. Without
                # this the coroutine would not start until the next `await`, which is after the
                # whole next extraction, and the overlap would not exist.
                for _ in range(4):
                    await asyncio.sleep(0)
                await asyncio.sleep(0.001)
            _prev_packed = packed_tensor

        # ---- drain the last in-flight apply (pipelined mode only; a no-op otherwise)
        if _pending is not None:
            await _pending
            _pending = None
        if _pipeline:
...
class CudaIpcWeightTransferReceiver(WeightTransferReceiver):
    ...
    def receive_weights(self, request: CudaIpcWeightUpdateRequest) -> Iterator[Tuple[str, torch.Tensor]]:
        ...
        # The chunk dtype need NOT be the model dtype. The IsoExec native sync ships allowlisted
        # buffers (e.g. the fp32 `mlp.router.expert_bias`, which decides MoE top-k membership) in
        # their native precision in their own dtype-homogeneous chunk; requiring model dtype here
        # forced them through bf16 and flipped routing. The consumer copies each tensor into its
        # destination with an explicit `.to(dest.dtype)`, so a non-model dtype lands correctly --
        # and for an fp32 source into an fp32 destination it lands EXACTLY, which is the point.
        # Only the homogeneity assumption above is load-bearing (one flat packed buffer).
