...
class PolicyWorkerBase(Worker):
    ...
    def forward(
        ...
        if loss_fn is None:
            ...
            # Uses token-based micro-batching when the forward token budget is > 0, otherwise
            ...
            # The budget is `max_tokens_per_microbatch_forward` when set, else the training
            # budget -- see `resolve_forward_token_budget`.
            microbatch_iterator = get_microbatch_iterator(
                data,
                micro_batch_size=self.cfg.micro_forward_batch_size_per_gpu,
                max_tokens_per_microbatch=resolve_forward_token_budget(self.cfg),
            )
...
class CriticWorkerBase(Worker):
    ...
    def forward(self, data: TrainingInputBatch) -> WorkerOutput:
        ...
        # Uses token-based micro-batching when the forward token budget is > 0; otherwise fixed
