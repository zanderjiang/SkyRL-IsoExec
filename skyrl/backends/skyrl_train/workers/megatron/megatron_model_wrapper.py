...
from skyrl.backends.skyrl_train.workers.worker_utils import (
    compute_post_update_rollout_logprob_diff_metrics,
)
from skyrl.train.config import TrainerConfig


def _isoexec_scoring_ctx():
    """SkyRL-IsoExec: run the Megatron forward/backward under the unified (vops) RMSNorm so the
    ...
    No-op when SKYRL_ISOEXEC != 1.
    Bisect toggle: SKYRL_ISOEXEC_SCORING_FORWARD=0 disables the vops-norm wrap (to test whether it
    is what breaks the trainer's Megatron forward -> near-uniform logits / entropy ~8)."""
    if os.environ.get("SKYRL_ISOEXEC") == "1" and os.environ.get("SKYRL_ISOEXEC_SCORING_FORWARD", "1") == "1":
...
def _isoexec_scoring_logits_admit(model_chunks, temperature: float, group=None) -> bool:
    """SkyRL-IsoExec: may the scoring forward keep its logits in the model's own precision?

    Thin, import-guarded front door for ``isoexec.runtimes.megatron.scoring_logits_dtype.admit`` so a
    tree without the IsoExec package (or a partially-installed one) keeps Megatron's unconditional
    fp32 upcast.  The local result and exact output dtype are MIN/MAX-voted over TP before the model
    forward: a rank-local import/config/env failure must make every rank retain fp32, while two
    different locally valid precisions must also refuse.  Otherwise the downstream vocabulary
    collective would receive different element widths and could corrupt or hang.
    """
    local_code = 0
    try:
        from skyrl.backends.skyrl_train.isoexec.runtimes.megatron.scoring_logits_dtype import (
            admit,
            admitted_output_dtype,
        )

        if admit(model_chunks, temperature):
            dtype = admitted_output_dtype(model_chunks)
            local_code = {torch.bfloat16: 1, torch.float16: 2}.get(dtype, 0)
    except Exception:  # noqa: BLE001 -- the common TP vote turns local failure into common refusal
        local_code = 0
    ...
    low = torch.tensor(local_code, dtype=torch.int32, device=device)
    high = low.clone()
    ...
    return bool(low.item() > 0 and low.item() == high.item())


def _isoexec_scoring_logits_kwargs(admitted: bool) -> dict:
    """``{"fp32_output": False}`` when admitted, else ``{}`` -- see :func:`_isoexec_scoring_logits_admit`."""
...
class MegatronModelWrapper:
    ...
    def forward(
        ...
        # SkyRL-IsoExec: decide ONCE, before any microbatch, whether this scoring forward may keep its
        # logits in the model's own precision instead of taking Megatron's last-stage fp32 upcast
        # (`Float16Module.forward`'s `float16_to_fp32(outputs)`). The verdict is structural -- env,
        # temperature, and the module chain -- so every rank reaches the same answer, which it must:
        # the dtype decided here is the `src_dtype` the full-vocab all_gather's bf16 wire admits on,
        # and a rank that disagreed would post a different byte count into that collective. See
        # isoexec/runtimes/megatron/scoring_logits_dtype.py for the bitwise argument.
        _ix_keep_logits_dtype = _isoexec_scoring_logits_admit(
            self.actor_module,
            temperature,
            group=mpu.get_tensor_model_parallel_group(),
        )
        _ix_fwd_kwargs = _isoexec_scoring_logits_kwargs(_ix_keep_logits_dtype)
        ...
        def forward_step(batch_iter, model):
            ...
            # SkyRL-IsoExec: pass attention_mask=None (pure causal flash) to match the engine's
            ...
            _ix_mask = (
                None if (os.environ.get("SKYRL_ISOEXEC") == "1" and packed_seq_params is None) else new_attention_mask
            )
            outputs = model(
                new_sequences,
                new_position_ids,
    ...
    def forward_backward_mini_batch(
        ...
        def loss_func(logits, data):
            ...
            metrics.update(
                compute_post_update_rollout_logprob_diff_metrics(action_log_probs, rollout_action_logprobs, loss_mask)
            )
            return loss, metrics
