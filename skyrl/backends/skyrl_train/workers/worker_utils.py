...
# Per-micro-batch abs diff between the policy-update forward and rollout logprobs. This is
# deliberately NOT named like the IsoExec gate: when a batch contains multiple sequential PPO
# mini-batches, later entries run after earlier optimizer steps and therefore include expected
# policy drift. The canonical identical-weight gate is computed by the controller's pre-update
# scoring phase and logged as ``policy/rollout_train_logprobs_abs_diff_*``.
#
# The moments (`_mean`, `_sq_mean`) and `_max`/`_min` reduce correctly across micro-batches, DP
# ranks, and mini-batches; the std is reconstructed from the moments downstream.
POST_UPDATE_ROLLOUT_LOGPROB_DIFF_PREFIX = "post_update_rollout_logprobs_abs_diff"
POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MEAN_KEY = f"{POST_UPDATE_ROLLOUT_LOGPROB_DIFF_PREFIX}_mean"
POST_UPDATE_ROLLOUT_LOGPROB_DIFF_SQ_MEAN_KEY = f"{POST_UPDATE_ROLLOUT_LOGPROB_DIFF_PREFIX}_sq_mean"
POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MAX_KEY = f"{POST_UPDATE_ROLLOUT_LOGPROB_DIFF_PREFIX}_max"
POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MIN_KEY = f"{POST_UPDATE_ROLLOUT_LOGPROB_DIFF_PREFIX}_min"
POST_UPDATE_ROLLOUT_LOGPROB_DIFF_STD_KEY = f"{POST_UPDATE_ROLLOUT_LOGPROB_DIFF_PREFIX}_std"

# Python-symbol compatibility for downstream imports. These aliases intentionally resolve to the
# new, honest metric names; the misleading ``minibatch_*`` tracker keys are no longer emitted.
MINIBATCH_ROLLOUT_LOGPROB_DIFF_PREFIX = POST_UPDATE_ROLLOUT_LOGPROB_DIFF_PREFIX
MINIBATCH_ROLLOUT_LOGPROB_DIFF_MEAN_KEY = POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MEAN_KEY
MINIBATCH_ROLLOUT_LOGPROB_DIFF_SQ_MEAN_KEY = POST_UPDATE_ROLLOUT_LOGPROB_DIFF_SQ_MEAN_KEY
MINIBATCH_ROLLOUT_LOGPROB_DIFF_MAX_KEY = POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MAX_KEY
MINIBATCH_ROLLOUT_LOGPROB_DIFF_MIN_KEY = POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MIN_KEY
MINIBATCH_ROLLOUT_LOGPROB_DIFF_STD_KEY = POST_UPDATE_ROLLOUT_LOGPROB_DIFF_STD_KEY


@torch.no_grad()
def compute_post_update_rollout_logprob_diff_metrics(
    action_log_probs: torch.Tensor,
    rollout_logprobs: Optional[torch.Tensor],
    loss_mask: Optional[torch.Tensor],
) -> Dict[str, float]:
    """Per-micro-batch abs diff between the policy-update forward and rollout logprobs.

    Unlike the pre-update scoring gate ``policy/rollout_train_logprobs_abs_diff_*``, this uses the
    logprobs the loss actually optimizes against. It therefore reflects expected policy drift when
    ``train_batch_size > mini_batch_size`` and optimizer steps occur between mini-batches. It must
    never be used as the IsoExec forward-identity gate. Masked-out tokens are excluded via the same
    ...
    return {
        POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MEAN_KEY: masked_mean(abs_diff, loss_mask).item(),
        POST_UPDATE_ROLLOUT_LOGPROB_DIFF_SQ_MEAN_KEY: masked_mean(abs_diff.square(), loss_mask).item(),
        POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MAX_KEY: masked_abs_diff.max().item(),
        POST_UPDATE_ROLLOUT_LOGPROB_DIFF_MIN_KEY: masked_abs_diff.min().item(),
    }


# Backward-compatible import name; emitted keys use ``post_update_*`` above.
compute_minibatch_rollout_logprob_diff_metrics = compute_post_update_rollout_logprob_diff_metrics
...
def resolve_forward_token_budget(cfg) -> int:
    """The token budget the FORWARD-ONLY (scoring / old-logprob) phase must pack to.

    One rule, read by every backend's forward path, so the scoring phase can never drift from the
    training phase by accident: ``trainer.max_tokens_per_microbatch_forward`` when set to a positive
    value, otherwise ``trainer.max_tokens_per_microbatch`` verbatim (including its ``-1`` = packing
    disabled). The packer itself is shared -- this only chooses the bin capacity handed to
    :func:`get_microbatch_iterator`; there is no second packing implementation anywhere.

    ``getattr`` with a default keeps this working against configs built before the field existed
    (resumed runs, hand-built test configs), where the answer is simply the training budget.
    """
    forward_budget = getattr(cfg, "max_tokens_per_microbatch_forward", -1)
    if forward_budget is not None and forward_budget > 0:
        return int(forward_budget)
    return int(cfg.max_tokens_per_microbatch)
