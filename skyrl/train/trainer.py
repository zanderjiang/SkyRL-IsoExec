...
from skyrl.backends.skyrl_train.utils.ep_balance import (
    ...
    inverse_permutation,
    permute_batch_rows,
    stripe_minibatch_permutation_for_dp,
...
class RayPPOTrainer:
    def __init__(
        ...
        # SkyRL-IsoExec wave-2C: sampled gating for the redundant scoring forward. N=1 (default)
        # keeps the historical behaviour -- the scoring forward runs every step. N>1 runs it only
        # on steps where ``global_step % N == 0`` (plus step 1 always); on the other steps the
        # rollout logprobs stand in for it. Resolved once here so a malformed value fails loudly at
        # construction rather than mid-run. See `_isoexec_sampled_gating_skip`.
        try:
            self._isoexec_scoring_audit_interval = int(os.environ.get("SKYRL_ISOEXEC_SCORING_AUDIT_INTERVAL", "1"))
        except ValueError as _e:
            raise ValueError(
                "SKYRL_ISOEXEC_SCORING_AUDIT_INTERVAL must be a positive integer "
                f"(got {os.environ.get('SKYRL_ISOEXEC_SCORING_AUDIT_INTERVAL')!r})"
            ) from _e
        if self._isoexec_scoring_audit_interval < 1:
            raise ValueError(
                f"SKYRL_ISOEXEC_SCORING_AUDIT_INTERVAL must be >= 1, got {self._isoexec_scoring_audit_interval}"
            )
        if self._isoexec_scoring_audit_interval > 1:
            # The interval being set is NOT the same as the skip being reachable: every guard in
            # `_isoexec_sampled_gating_skip` can veto it, and the config/env-decidable ones are
            # decidable right here. Say so at construction. Without this the banner below printed
            # "INSTALLED ... every 10 steps" on arm 6kv38f6e while the skip never once fired
            # (`policy/scoring_forward_skipped` == 0.0 on every step) because the matched arm had
            # pinned `off_policy_correction.tis_ratio_type=token` from the native baseline. That
            # cost ~944 s/step for the life of the run and took a wandb config diff to find --
            # exactly the "absence of a banner is not evidence" trap, inverted.
            #
            # ONE PREDICATE, TWO READERS: this banner and the per-step skip both call
            # `_isoexec_sampled_gating_static_vetoes`, so a veto added there cannot drift out of the
            # banner (which is how this trap was set in the first place).
            _ix_vetoes = self._isoexec_sampled_gating_static_vetoes()
            if _ix_vetoes:
                logger.warning(
                    f"[ISOEXEC-SAMPLED-GATING] INERT: SKYRL_ISOEXEC_SCORING_AUDIT_INTERVAL="
                    f"{self._isoexec_scoring_audit_interval} was set, but the skip can NEVER fire because: "
                    + "; ".join(_ix_vetoes)
                    + ". The scoring forward will run on EVERY step (watch "
                    "`timing/fwd_logprobs_values_reward`). Clear the key above or unset the interval."
                )
            else:
                logger.info(
                    f"[ISOEXEC-SAMPLED-GATING] INSTALLED: scoring forward runs on step 1 and every "
                    f"{self._isoexec_scoring_audit_interval} steps; rollout logprobs substituted otherwise. "
                    f"`policy/rollout_train_logprobs_abs_diff_*` is emitted ONLY on audit steps. "
                    f"Non-audit steps emit `policy/isoexec_forward_gate_audited=0`; canonical gate "
                    f"values stay absent rather than being faked."
                )
        else:
            logger.info("[ISOEXEC-SAMPLED-GATING] SKIPPED: audit interval 1 (scoring forward every step)")
        if os.environ.get("SKYRL_ISOEXEC") == "1":
            logger.info(
                "[ISOEXEC-GATE] ARMED: canonical `policy/rollout_train_logprobs_abs_diff_*` "
                "comes only from the pre-update scoring forward at rollout-identical weights; "
                "`policy/post_update_rollout_logprobs_abs_diff_*` is sequential-PPO policy "
                "drift, not a gate. Audited steps require a complete finite canonical gate; "
                "configured sampled non-audit steps emit `policy/isoexec_forward_gate_audited=0`."
            )
            ...
            for _, rand_prompts in enumerate(self.train_dataloader):
                ...
                with Timer("step", self.all_timings):
                    ...
                    if self.colocate_all:
                        ...
                        # TIMED (isoexec worklist F6): this interval sat between Timer("generate")
                        # and Timer("postprocess_generator_output") with no Timer of its own, so the
                        # +8.7-9.0 s/step it costs on GLM-4.7-Flash -- the third-largest term in
                        # that step gap -- could only ever be RECONSTRUCTED by subtracting logged
                        # timestamps, and on Qwen3.5-35B it had never been measured at all. It is a
                        # stage like any other; it gets a stage timer like any other.
                        with Timer("engine_sleep", self.all_timings):
                    ...
                    gate_audited = validate_isoexec_forward_gate(
                        self.all_metrics,
                        enabled=os.environ.get("SKYRL_ISOEXEC") == "1",
                        scoring_audit_skipped=self.all_metrics.get("policy/scoring_forward_skipped") == 1.0,
                    )
                    if gate_audited:
                        logger.info(
                            "[ISOEXEC-GATE] SERVED step={} source=pre_update_scoring mean={:.3e} max={:.3e}",
                            self.global_step,
                        ...
                        logger.info(
                            ...
                            "canonical gate metrics intentionally absent",
                            self.global_step,
                        )
    ...
    def convert_to_training_input(self, generator_output: GeneratorOutput, uids: List[str]) -> TrainingInputBatch:
        ...
        # MAX, not just the mean: the engine runs as many DECODE STEPS as the LONGEST sequence in
        # the batch, so max_response_length -- not the mean -- is what generate wall-time is
        # proportional to. Normalising perf numbers by the mean is misspecified, and it hid the
        # variance in every generate measurement taken on 2026-07-19 (the IsoExec arm's 5.2s fit
        # residual vs native's 0.84s is the same max/mean spread amplified by a 5.3x per-step cost).
        training_input.metadata["max_response_length"] = max(
            len(sample_response_ids) for sample_response_ids in response_ids
        )
    ...
    def compute_advantages_and_returns(self, data: TrainingInputBatch) -> TrainingInputBatch:
        ...
        max_response_length = data.metadata.get("max_response_length", 0)
        data = data.to("cpu")
        ...
        data.metadata["metrics"].update(
            {
                ...
                # Divide generate wall-time by THIS, not the mean -- see the comment where it is set.
        ...
        logger.info(
            f"avg_final_rewards: {avg_rewards}, avg_response_length: {avg_response_length}, "
    ...
    def _execute_forward_pass(
        ...
        if (
            model == "policy"
            and ep_balance_enabled()
            and self.cfg.trainer.recompute_old_logprobs_per_minibatch
            and mini_batch_boundaries
        ):
            # Scoring used to run on the original generation-order DP shards while the training
            # step below striped the SAME mini-batches by real length. Production logs measured
            # 1.15-1.23x as many scoring bins and up to 1.15x as many critical-path tokens as the
            # balanced training schedule. Stage scoring in the identical deterministic order, then
            # restore its per-sample outputs before the rollout-vs-trainer gate. [FWD-N]: this
            # relies on the already-admitted batch-invariant forward and fails loudly at step 1 if
            # any per-token value changes.
            perm, stripe_stats = stripe_minibatch_permutation_for_dp(
                data_fwd_pass, mini_batch_boundaries, self.dispatch.dp_size(model)
            )
            if perm is not None:
                data_fwd_pass = permute_batch_rows(data_fwd_pass, perm)
                restore_order = inverse_permutation(perm)
                logger.info(
                    f"[ISOEXEC-EPBAL] {model} scoring stripe: "
                    f"served_ranges={stripe_stats['applied_ranges']}/{stripe_stats['total_ranges']}, "
                    f"per-rank real-token spread {stripe_stats['rank_token_spread_before']} -> "
                    f"{stripe_stats['rank_token_spread_after']}"
                )
        ...
        if self.cfg.trainer.recompute_old_logprobs_per_minibatch and mini_batch_boundaries:
            ...
            outputs = loss_fn_outputs_to_tensor(combined_outputs, key=key)
            return outputs if restore_order is None else outputs[restore_order]

        output = self.dispatch.forward(model, data_fwd_pass)
        outputs = loss_fn_outputs_to_tensor(output.loss_fn_outputs, key=key)
        return outputs if restore_order is None else outputs[restore_order]
    ...
    def _isoexec_sampled_gating_static_vetoes(self) -> List[str]:
        """Every reason the scoring-forward skip can NEVER fire that is decidable from config/env.

        THE SINGLE SOURCE for both readers: `_isoexec_sampled_gating_skip` (which refuses when this
        is non-empty) and the construction banner (which prints it). Adding a static guard to the
        skip means adding it here, and the banner follows for free -- the hand-copied version of
        this list is exactly what let arm 6kv38f6e advertise a skip that never fired.

        Guards that are NOT static -- rollout logprobs actually present in the batch, a ref model
        attached by `build_models` after construction, and "step 1 is always audited" -- stay in
        the per-step predicate; they cannot be answered here.
        """
        vetoes: List[str] = []
        if os.environ.get("SKYRL_ISOEXEC") != "1":
            vetoes.append(
        ...
        if off_policy_correction_enabled(algorithm.off_policy_correction):
            vetoes.append(
                "trainer.algorithm.off_policy_correction is ENABLED "
                f"(tis_ratio_type={algorithm.off_policy_correction.tis_ratio_type!r}, "
                f"sequence_mask_metric={algorithm.off_policy_correction.sequence_mask_metric!r}) "
                "-- TIS/masking read the scoring forward's old logprobs, so the skip is refused"
            )
        if self.has_critic:
            vetoes.append(f"a critic is configured (trainer.critic.model.path={self.cfg.trainer.critic.model.path!r})")
        return vetoes

    def _isoexec_sampled_gating_skip(self, training_input: TrainingInputBatch) -> bool:
        """SkyRL-IsoExec wave-2C: skip the redundant scoring forward on non-audit steps.

        Under IsoExec the scoring forward's "old" logprobs and the rollout logprobs the engine
        already returned agree to the ~6.6e-07 cross-runtime floor -- that agreement is the whole
        point of the stack, and it is what the IsoExec gate measures. So on most steps this phase is
        ~46 s of GPU time spent recomputing a tensor we are already holding. This substitutes the
        rollout logprobs for the scoring forward's output on non-audit steps.

        SAFETY / WHAT THIS DOES *NOT* TOUCH:
          * The canonical gate is ``policy/rollout_train_logprobs_abs_diff_mean`` from this
            pre-update scoring phase. A skipped scoring forward cannot produce that gate and is
            explicitly marked ``policy/isoexec_forward_gate_audited=0``. The train-step
            metric is retained separately as ``policy/post_update_rollout_logprobs_abs_diff_*``;
            it includes expected drift after sequential PPO mini-batch optimizer steps.
          * The train-step forward is bit-unchanged: ``old_action_log_probs`` only enters the LOSS
            (the PPO ratio), never the forward. Advantages are reward-only under GRPO.
          * ``policy/rollout_train_logprobs_abs_diff_*`` (this phase's own metric) is genuinely
            unavailable on a skipped step and is therefore NOT emitted -- never faked to 0.0.
            ``policy/scoring_forward_skipped`` (1.0/0.0) records which steps were audited.

        Guards mirror ``_skip_policy_forward``: rollout logprobs must exist, the KL reward penalty
        and off-policy correction must be off (both read the old logprobs for real), and there must
        be no critic/ref consumer of them. Step 1 ALWAYS runs the full forward.
        """
        interval = self._isoexec_scoring_audit_interval
        if interval <= 1:
            return False
        # The config/env-decidable guards live in ONE place, shared with the construction banner --
        # SKYRL_ISOEXEC, the KL reward penalty, off-policy correction, a configured critic.
        if self._isoexec_sampled_gating_static_vetoes():
            return False
        if training_input.get("rollout_logprobs", None) is None:
            return False
        if self.ref_model is not None:
            # `apply_reward_kl_penalty` / value clipping read the old logprobs against a second
            # model; substituting there would change more than the ratio. Not our case, but the
            # guard keeps the knob inert if a ref/critic is ever added.
            return False
        if self.global_step <= 1:
            return False  # step 1 is always an audit step (the frozen-signature step)
        return (self.global_step % interval) != 0
    ...
    def fwd_logprobs_values_reward(
        ...
        isoexec_substitute_rollout = False
        if self._skip_policy_forward(training_input):
            action_log_probs = None
        elif self._isoexec_sampled_gating_skip(training_input):
            # SkyRL-IsoExec wave-2C sampled gating: no forward this step; the rollout logprobs are
            # substituted below (after this phase's own gate metric, which is deliberately not
            # emitted on a skipped step). See `_isoexec_sampled_gating_skip`.
            action_log_probs = None
            isoexec_substitute_rollout = True
        else:
        ...
        if self._isoexec_scoring_audit_interval > 1:
            # Sampled gating bookkeeping. 1.0 = this step's scoring forward was skipped and the
            # rollout logprobs stand in as "old"; 0.0 = full audit step (the phase ran, and
            # `policy/rollout_train_logprobs_abs_diff_*` below is real). Never faked.
            self.all_metrics["policy/scoring_forward_skipped"] = 1.0 if isoexec_substitute_rollout else 0.0
        if isoexec_substitute_rollout:
            # Same shape/dtype/alignment as the forward's output: both are [B, response_length]
            # float32 CPU, right-aligned over the response window. Cloned so downstream in-place
            # edits cannot alias the rollout tensor the loss also reads as `rollout_logprobs`.
            training_input["action_log_probs"] = training_input["rollout_logprobs"].to(torch.float32).clone()
            logger.info(
                f"[ISOEXEC-SAMPLED-GATING] step {self.global_step}: scoring forward SKIPPED "
                f"(audit interval {self._isoexec_scoring_audit_interval}); rollout logprobs used as "
                f"old-logprobs. Canonical `rollout_train_logprobs_abs_diff_*` is absent and this "
                f"step will be marked `policy/isoexec_forward_gate_audited=0`."
            )
        ...
        if training_input.get("rollout_logprobs", None) is not None and action_log_probs is not None:
            ...
            # forward pass is skipped, the canonical gate is unavailable and the controller marks
            # this as a sampled non-audit step rather than fabricating a value.
            logprobs_diff = (
            ...
            if _os_zk.environ.get("SKYRL_ISOEXEC") == "1":
                ...
                # SCIENTIFIC notation + an exact-zero COUNT. The old format (mean to 5 decimals,
                # diffs rounded to 3) rendered a 3.5e-07 mean as "0.00000" and a 6e-06 max as "0.0"
                # -- it could not tell bitwise-zero from the ~1e-6 cross-runtime floor, and that
                # artifact is what "35B is exact 0.0" rested on. Report `exact` (the count of
                # genuinely bit-identical tokens), which is the claim IsoExec actually makes.
                _exact = int((_d == 0).sum())
                print(
                    f"[ISOEXEC-DIFF] n={_d.numel()} exact={_exact}/{_d.numel()} "
                    f"mean={_d.mean():.3e} max={_d.max():.3e} frac>0.05={_frac_big:.3%} "
                    f"| top8 diffs={[f'{x:.2e}' for x in _topv.tolist()]} "
                    f"rollout@top={[round(x, 3) for x in _roll[_topi].tolist()]} "
                    f"trainer@top={[round(x, 3) for x in _act[_topi].tolist()]}",
                    flush=True,
                )
            self.all_metrics.update(
                {
    ...
    def _execute_training_step(self, model: str, data: TrainingInputBatch) -> Dict[str, float]:
        ...
        # SkyRL-IsoExec EP-skew balance (SKYRL_ISOEXEC_TRAINER_EP_BALANCE, default OFF): within each
        # mini-batch, stripe rows across DP ranks by real length so every rank's shard carries a
        # near-identical length profile -- at EP spanning the world, every MoE layer of every
        # microbatch is an all-rank rendezvous that serializes on the slowest rank, and contiguous
        # generation-order chunking makes rank shard costs wildly unequal (probe B: 81.4% of all
        # trainer kernel time was that wait). Mini-batch MEMBERSHIP is untouched; only rank
        # assignment and fp32 accumulation grouping move (trajectory-rounding class -- mechanism and
        # the full bitwise legality argument in backends/skyrl_train/utils/ep_balance.py). This path
        # consumes all-reduced metrics, never per-sample outputs, so row order is not load-bearing.
        # Policy scoring independently applies the same stripe and restores its positional outputs.
        if ep_balance_enabled():
            data, _epbal_stats = stripe_minibatch_rows_for_dp(data, boundaries, self.dispatch.dp_size(model))
            if _epbal_stats is not None:
                logger.info(
                    f"[ISOEXEC-EPBAL] {model} trainer stripe: "
                    f"{_epbal_stats['applied_ranges']}/{_epbal_stats['total_ranges']} mini-batches, "
                    f"per-rank real-token spread {_epbal_stats['rank_token_spread_before']} -> "
                    f"{_epbal_stats['rank_token_spread_after']}"
                )
        ...
        finalize_post_update_rollout_logprob_diff_std(reduced_metrics)
    ...
    def train_critic_and_policy(self, data: TrainingInputBatch):
        ...
        reject_isoexec_gate_metric_collisions(policy_status)
