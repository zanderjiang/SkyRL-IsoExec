...
class AgentLoopState:
    ...
    # was the sole per-sequence rollout-vs-train (IsoExec) outlier on EOS-terminated sequences.
...
class SkyRLGymGenerator(GeneratorInterface):
            ...
            if not self.use_conversation_multi_turn:
                ...
                if stop_reason != "length" and response_ids and response_ids[-1] != self.tokenizer.eos_token_id:
                    ...
                    if rollout_logprobs is not None:
                        ...
                        # placeholder was the per-sequence IsoExec outlier: the trainer scores a real
    ...
    def _update_agent_loop_state_with_singleturn_chat_template(
        ...
        if new_resp_tokens and new_resp_tokens[-1] == self.tokenizer.eos_token_id:
            ...
            if turn_output.output_logprobs is not None and len(turn_output.output_logprobs) == len(new_resp_tokens):
